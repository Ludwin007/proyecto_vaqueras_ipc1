...
@WebServlet(name = "LoginGamerController", urlPatterns = {"/api/login/gamer"})
public class LoginGamerController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (loginDTO.getCorreo() == null || loginDTO.getCorreo().isEmpty()) {
                ResponseDTO errorResponse = new ResponseDTO(false, "Opps, el correo o nickname es obligatorio");
            ...
            GamersDTO gamer = autenticacionService.loginGamer(
                loginDTO.getCorreo(), 
            ...
            if (gamer != null) {
                ...
                session.setAttribute("tipoUsuario", "gamer");
                session.setAttribute("usuario", gamer);
                session.setAttribute("idUsuario", gamer.getIdUsuario());
                
                ResponseDTO successResponse = new ResponseDTO(true, "Listo, login exitoso", gamer);
                response.setStatus(HttpServletResponse.SC_OK);
                response.getWriter().write(gson.toJson(successResponse));
                
            } else {
                ResponseDTO errorResponse = new ResponseDTO(false, "Opps, tus credenciales son incorrectas");
                response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
                response.getWriter().write(gson.toJson(errorResponse));
            }
            
        } catch (Exception e) {
            System.err.println("Error en LoginGamerController: " + e.getMessage());
            e.printStackTrace();
